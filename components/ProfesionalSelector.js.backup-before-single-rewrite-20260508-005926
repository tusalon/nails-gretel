        ...
        try {
            ...
            if (window.getProfesionalesPorServicio) {
                ...
                const listasPorServicio = await Promise.all(
                    serviciosSeleccionados.map(servicio => window.getProfesionalesPorServicio(servicio.id))
                );
                const idsDelServicio = listasPorServicio.reduce((idsComunes, lista, index) => {
                    const idsActuales = lista.map(p => p.id);
                    if (index === 0) return idsActuales;
                    return idsComunes.filter(id => idsActuales.includes(id));
                }, []);
